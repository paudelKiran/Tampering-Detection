...
def build_dataset(image_paths, mask_paths, batch_size, shuffle=False):
...
def get_datasets(manifest_dir="manifests", batch_size=8, project_root=None):
    ...
    train_imgs, train_masks = load_manifest(
        os.path.join(manifest_dir, "train_manifest.csv"), project_root
    )
    val_imgs, val_masks = load_manifest(
        os.path.join(manifest_dir, "val_manifest.csv"), project_root
    )
    test_imgs, test_masks = load_manifest(
        os.path.join(manifest_dir, "test_manifest.csv"), project_root
    )

    train_dataset = build_dataset(train_imgs, train_masks, batch_size, shuffle=True)
    val_dataset = build_dataset(val_imgs, val_masks, batch_size, shuffle=False)
    test_dataset = build_dataset(test_imgs, test_masks, batch_size, shuffle=False)
...
if __name__ == "__main__":
    ...
    for images, masks in train_ds.take(1):
        ...
        print(f"Mask unique vals:  {sorted(set(masks.numpy().flatten()))}")
